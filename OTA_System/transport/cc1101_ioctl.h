 ...
 *
 * [gateway-ota 쪽 사본, 2026-08-22 갱신] kernel-cc1101-spi/cc1101_ioctl.h(develop
 * 브랜치, FHSS 채널 호핑 반영분)와 반드시 내용이 같아야 하는 계약 헤더입니다
 * (ota_protocol.h가 gateway-ota·firmware-esp32 사이의 계약인 것과 같은 구조).
 * 커널 쪽 헤더가 바뀌면 이 파일도 같이 갱신해야 합니다. 이번 갱신으로 FHSS
 * 관련 구조체(cc1101_fhss_rf_profile/hop_policy/config/status)와 ioctl
 * 14~17번(SET_CONFIG/START/STOP/GET_STATUS)이 새로 추가됐습니다 — 기존
 * OTA 전용 ioctl 0~13번은 번호·의미 그대로 유지.
 */
