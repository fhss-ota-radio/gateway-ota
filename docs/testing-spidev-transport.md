...
## 4. 알려진 제약 — 일부 패킷 유실은 "정상"입니다

이 스모크테스트(`performHandshake()` + `sendDataAndEnd()`)는 **DATA 구간에
청크 단위 재전송이 없습니다.** 핸드셰이크(`OTA_START`)만 응답을 기다리고
재시도하고, 그 뒤 DATA는 응답 확인 없이 전부 순서대로 쏘기만 합니다
(`session/simplesender.cpp`의 `sendDataAndEnd()` 참고). 이건 버그가 아니라
`docs/file-transfer-guide.md` 8절에 이미 문서화된 현재 구현 범위입니다:

| 상황 | 필요 처리 | 현재 구현 여부 |
|---|---|---|
| 일부 패킷 유실 | NACK/재전송으로 채움 | **미구현** |

그래서 실기기 무선 환경에서 수신 쪽 로그의 "실제 받은 DATA 개수"가 송신
쪽 "총 청크 수"보다 적게 나오는 건 예상된 결과입니다 (2026-08-16 테스트:
1067개 중 838개 수신). 재전송 큐는 README 마일스톤 4 / `OtaSession`(FSM)
몫으로 아직 계획 단계입니다 — 지금 이 스모크테스트의 목적은 "재전송까지
포함한 신뢰성 있는 전송"이 아니라 "핸드셰이크→DATA→END 흐름 자체와 무선
링크가 물리적으로 동작하는지" 확인하는 것입니다.
